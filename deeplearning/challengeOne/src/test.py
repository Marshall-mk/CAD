...
def main(cfg):
    ...
    for model_name in all_models:
        ...
        all_image_names = []
        with torch.no_grad():
            for inputs, image_names in test_dataloader:
                inputs = inputs.to(device)  # Get only the images
                outputs = model(inputs)
                probs = torch.sigmoid(outputs).cpu().numpy()
                preds = (torch.sigmoid(outputs) > 0.5).cpu().numpy()
                # change the preds to int
        ...
        image_names_file = f"../metrics/{model_name}_image_names.txt"
        np.savetxt(image_names_file, all_image_names, fmt='%s') 
